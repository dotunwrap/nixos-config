...
let
  cfg = config.dotunwrap.homeConfigurations;
...
{
  options = {
    dotunwrap.homeConfigurations = lib.mkOption {
      type = lib.types.attrsOf (
        lib.types.submodule (
          ...
          {
            options = {
              enable = lib.mkEnableOption "Enable this home configuration." // {
                default = true;
              };
            ...
            config = lib.mkIf config.enable {
              entryPoint = import "${self}/home/configurations/${config.username}-at-${config.hostname}.nix" (
                inputs // { inherit self; }
              );
              base = "home";
