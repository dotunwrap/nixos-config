...
let
  ...
  callModule =
    module: args:
    let
      moduleToImport = if args == null then module else import module args;
    in
    {
      imports = [ moduleToImport ];
    }
    // lib.optionalAttrs (builtins.any (p: p module) [
      isPath
      isString
    ]) { _file = module; };

  from = lib.types.oneOf [
    lib.types.str
    lib.types.path
  ];

  submodule = lib.types.coercedTo from (m: { module = m; }) (
    lib.types.submodule (
      {
        name,
        config,
